...
const ThankYou = () => {
  ...
  const nombre = location.state?.nombre || localStorage.getItem('nombreComprador') || "Agradecedor/a consciente";
  const ebookId = location.state?.ebookId || localStorage.getItem('idComprado');
  ...
  const calcularPrecioDescuento = (precioString) => {
    // Asumimos que el precio viene como "$ 5.000" o "5000". Lo limpiamos.
    // Si tus precios en data tienen puntos, ej: "5.000", esto funciona:
    const numeroLimpio = precioString.replace(/[^0-9]/g, ''); 
    const precioOriginal = parseInt(numeroLimpio);
    const precioConDescuento = precioOriginal * 0.80; // 20% OFF
    
    // Formateamos de nuevo a moneda (Ej: $ 4.000)
      ...
      <div className="container mx-auto px-6 text-center">
        ...
        <div className="max-w-2xl mx-auto bg-white rounded-3xl p-8 border border-green-100 flex flex-col md:flex-row items-center gap-8 mb-20 shadow-xl transform transition hover:scale-[1.01]">
          ...
          <div className="text-left flex-1">
            ...
            <h3 className="text-2xl font-serif text-green-950 mb-6 leading-tight">
              {ebookComprado?.titulo || "Ebook de gratitud"}
            </h3>
            
            {/* BOTÓN DESCARGAR */}
            <a href={ebookComprado?.pdfLink} download className="btn-descarga">
          ...
          <div className="relative z-10">
            ...
            <div className="flex overflow-x-auto pb-8 gap-6 snap-x snap-mandatory scrollbar-hide">
              {otrosEbooks.map((item) => (
                // Agregamos 'flex flex-col h-full' para que todas las tarjetas tengan la misma altura y el contenido se distribuya
                <div key={item.id} className="min-w-[260px] md:min-w-[280px] bg-white rounded-2xl p-4 shadow-sm hover:shadow-xl transition duration-300 snap-center border border-emerald-50 group flex flex-col justify-between h-auto">
                  ...
                  <div>
                    ...
                    <div className="text-left mb-4">
                      {/* TITULO CORREGIDO: Sin overflow hidden, altura flexible */}
                      <h4 className="font-serif text-green-900 text-lg leading-tight mb-3">
                        {item.titulo}
                      </h4>
                      
                      {/* PRECIOS CORREGIDOS: Uno arriba del otro */}
        ...
        <div className="mt-20 pb-4 flex flex-col items-center opacity-70">
          ...
          <div className="flex gap-8 mb-6">
             <a href="https://wa.me/message/FBYSYJ6OAT77G1" target="_blank" className="text-emerald-700 font-bold text-sm hover:underline tracking-widest uppercase">WhatsApp</a>
             <a href="https://instagram.com/carito.descubrimientodelser" target="_blank" className="text-pink-700 font-bold text-sm hover:underline tracking-widest uppercase">Instagram</a>
          </div>
          {/* <p className="text-[9px] text-gray-400 tracking-[4px] uppercase">Diseño con ❤️ por @socialbysel</p> */}
        </div>
      </div>
    </div>
  );
